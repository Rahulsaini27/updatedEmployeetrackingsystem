...
def add_emp(request):
    if request.method == 'POST':
        ...
        role_id = request.POST.get('role_id')  # ✅ Get role ID from form
        ...
        try:
            empObj = Employee.objects.create(
                ...
                e_address=e_address,
                o_id_id=o_id,
                role_id=role_id if role_id else None  # ✅ Assign role if selected
            )
    ...
    # ✅ Pass all available roles to template
    roles = Role.objects.all()
    return render(request, 'AddEmp.html', {'roles': roles})



# ended changes in employee


# added role and permission 
...
def create_permission(request):
    ...
    # If GET, render the form page with permissions
    permissions = Permission.objects.all()
...
def update_emp(request, eid):
    try:
        emp_detail = get_object_or_404(Employee, id=eid, o_id_id=request.session['o_id'])
